...
class Decoder(Layer):
    ...
    def forward(self, inputs_prev, inputs):
        ...
        x = fluid.layers.concat([inputs_prev, x], axis = 1) # concat on C 
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.conv2(x)
        x = self.bn2(x)
...
class UNet(Layer):
    def __init__(self, num_classes=59):
        ...
        self.mid_conv1 = Conv2D(num_channels=512, num_filters=1024, filter_size=1, padding = 0, stride = 1)
        self.mid_bn1 = BatchNorm(num_channels=1024, act = 'relu')
        self.mid_conv2 = Conv2D(num_channels=1024, num_filters=1024, filter_size=1, padding = 0, stride = 1)
        ...
        # feature map size down to number of classes

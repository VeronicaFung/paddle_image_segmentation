...
class ASPPPooling(Layer):
    ...
    def __init__(self, num_channels, num_filters):
        ...
        self.features = fluid.dygraph.Sequential(
            Conv2D(num_channels, num_filters, filter_size = 1),
            BatchNorm(num_filters, act = 'relu')
        )

    def forward(self, inputs):
        n,c,h,w = inputs.shape
        x = fluid.layers.adaptive_pool2d(inputs,pool_size = 1)
        x = self.features(x)
...
class ASPPModule(Layer):
    ...
    def __init__(self, num_channels, num_filters, rates):
        ...
        self.project = fluid.dygraph.Sequential(
            Conv2D(num_filters*(2+ len(rates)), num_filters, filter_size = 1),
            BatchNorm(num_filters, act='relu')
        )
        for r in rates:
            self.features.append(
...
class DeepLab(Layer):
    ...
    def __init__(self, num_classes = 59):
        ...
        feature_dim = 2048
        self.classfier = DeepLabHead(feature_dim, num_classes)
